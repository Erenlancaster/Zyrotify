...
namespace god_checker
{
    ...
    internal class OnProgramStart
    {
        ...
        public static void Initialize(string name, string aid, string secret, string version)
        {
            ...
            {
                System.Windows.Forms.MessageBox.Show("Invalid application information!", Name);
            ...
            using (WebClient wc = new WebClient())
            {
                ...
                try
                {
                    ...
                    {
                        System.Windows.Forms.MessageBox.Show("Possible malicious activity detected!", OnProgramStart.Name, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        Process.GetCurrentProcess().Kill();
                    }
                    ...
                    {
                        System.Windows.Forms.MessageBox.Show("Security error has been triggered!", Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                        Process.GetCurrentProcess().Kill();
                    }
                    switch (response[2])
                    {
                        case "success":
                            ...
                            {
                                System.Windows.Forms.MessageBox.Show("Application is in Developer Mode, bypassing integrity and update check!", Name, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                                ...
                                System.Windows.Forms.MessageBox.Show("Your applications hash has been saved to integrity.txt, please refer to this when your application is ready for release!", Name, MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }
                            else
                            {
                               /* if (ApplicationSettings.Version != Version)
                                {
                                    System.Windows.Forms.MessageBox.Show($"Update {ApplicationSettings.Version} available, redirecting to update!", Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                                ...
                                if (response[12] == "Enabled")
                                {
                                    ...
                                    {
                                        System.Windows.Forms.MessageBox.Show($"File has been tampered with, couldn't verify integrity!", Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                                        Process.GetCurrentProcess().Kill();
                                    }
                                }*/
                            }
                            ...
                            {
                                System.Windows.Forms.MessageBox.Show("Looks like this application is disabled, please try again later!", Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                                Process.GetCurrentProcess().Kill();
                            }
                            break;
                        case "binderror":
                            System.Windows.Forms.MessageBox.Show(Encryption.Decode("RmFpbGVkIHRvIGJpbmQgdG8gc2VydmVyLCBjaGVjayB5b3VyIEFJRCAmIFNlY3JldCBpbiB5b3VyIGNvZGUh"), Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                            Process.GetCurrentProcess().Kill();
                            return;
                        case "banned":
                            System.Windows.Forms.MessageBox.Show("This application has been banned for violating the TOS" + Environment.NewLine + "Contact us at support@auth.gg", Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                            Process.GetCurrentProcess().Kill();
                            return;
                    }
                    Security.End();
                ...
                {
                    System.Windows.Forms.MessageBox.Show(ex.Message, Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Process.GetCurrentProcess().Kill();
                }
            }
        }
    ...
    internal class API
    {
        public static void Log(string username, string action)
        {
            ...
            {
                System.Windows.Forms.MessageBox.Show("Please initialize your application first!", OnProgramStart.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                Process.GetCurrentProcess().Kill();
            }
            if (string.IsNullOrWhiteSpace(action))
            {
                System.Windows.Forms.MessageBox.Show("Missing log information!", ApplicationSettings.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                Process.GetCurrentProcess().Kill();
            }
            string[] response = new string[] { };
            using (WebClient wc = new WebClient())
            {
                ...
                {
                    System.Windows.Forms.MessageBox.Show(ex.Message, OnProgramStart.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Process.GetCurrentProcess().Kill();
                }
            }
        }
        ...
        public static bool AIOLogin(string AIO)
        {
            ...
            {
                System.Windows.Forms.MessageBox.Show("Missing user login information!", ApplicationSettings.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                Process.GetCurrentProcess().Kill();
            }
            string[] response = new string[] { };
            using (WebClient wc = new WebClient())
            {
                try
                {
                    ...
                    if (response[0] != Constants.Token)
                    {
                        System.Windows.Forms.MessageBox.Show("Security error has been triggered!", OnProgramStart.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                        Process.GetCurrentProcess().Kill();
                    }
                    if (Security.MaliciousCheck(response[1]))
                    {
                    ...
                    switch (response[2])
                    {
                        ...
                        case "invalid_details":
                            ...
                            return false;
                        case "time_expired":
                            System.Windows.Forms.MessageBox.Show("Your subscription has expired!", ApplicationSettings.Name, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                        case "hwid_updated":
                            System.Windows.Forms.MessageBox.Show("New machine has been binded, re-open the application!", ApplicationSettings.Name, MessageBoxButtons.OK, MessageBoxIcon.Information);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                        case "invalid_hwid":
                            System.Windows.Forms.MessageBox.Show("This user is binded to another computer, please contact support!", ApplicationSettings.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                    }
                ...
                {
                    System.Windows.Forms.MessageBox.Show(ex.Message, ApplicationSettings.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    Security.End();
                    Process.GetCurrentProcess().Kill();
                }
                return false;
        ...
        public static bool AIORegister(string AIO)
        {
            ...
            if (string.IsNullOrWhiteSpace(AIO))
            {
                System.Windows.Forms.MessageBox.Show("Invalid registrar information!", ApplicationSettings.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                Process.GetCurrentProcess().Kill();
            }
            string[] response = new string[] { };
            using (WebClient wc = new WebClient())
                ...
                try
                {
                    ...
                    switch (response[2])
                    {
                        case "success":
                            ...
                            return true;
                        case "invalid_details":
                            System.Windows.Forms.MessageBox.Show("Sorry, your username/password does not match!", ApplicationSettings.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                        case "time_expired":
                ...
                try
                {
                    ...
                    switch (response[2])
                    {
                        ...
                        case "invalid_license":
                            System.Windows.Forms.MessageBox.Show("License does not exist!", ApplicationSettings.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                        case "email_used":
                            System.Windows.Forms.MessageBox.Show("Email has already been used!", ApplicationSettings.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                        case "invalid_username":
                            System.Windows.Forms.MessageBox.Show("You entered an invalid/used username!", ApplicationSettings.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                    }
                ...
                try
                {
                    ...
                    switch (response[2])
                    {
                        case "success":
                            ...
                            return true;
                        case "invalid_token":
                            System.Windows.Forms.MessageBox.Show("Token does not exist!", ApplicationSettings.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                        case "invalid_details":
                            System.Windows.Forms.MessageBox.Show("Your user details are invalid!", ApplicationSettings.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                            Security.End();
                            Process.GetCurrentProcess().Kill();
                            return false;
                    }
    ...
    internal class Security
    {
        ...
        public static void Start()
        {
            ...
            {
                System.Windows.Forms.MessageBox.Show("A session has already been started, please end the previous one!", OnProgramStart.Name, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                Process.GetCurrentProcess().Kill();
            }
            else
            {
                using (StreamReader sr = new StreamReader($@"{drive}Windows\System32\drivers\etc\hosts"))
                {
                    ...
                    {
                        Constants.Breached = true;
                        System.Windows.Forms.MessageBox.Show("DNS redirecting has been detected!", OnProgramStart.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                        Process.GetCurrentProcess().Kill();
                    }
                }
        ...
        public static void End()
        {
            if (!Constants.Started)
            {
                System.Windows.Forms.MessageBox.Show("No session has been started, closing for security reasons!", OnProgramStart.Name, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                Process.GetCurrentProcess().Kill();
            }
            else
            {
    ...
    class InfoManager
    {
        ...
        private void OnCallBack()
        {
            ...
            {
                Constants.Breached = true;
                System.Windows.Forms.MessageBox.Show("ARP Cache poisoning has been detected!", OnProgramStart.Name, MessageBoxButtons.OK, MessageBoxIcon.Error);
                Process.GetCurrentProcess().Kill();
            }
            else
            {
