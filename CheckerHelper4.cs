...
namespace god_checker
{
    internal class CheckerHelper4
    {
        ...
        public static void UpdateTitle()
        {
            for (; ; )
            {
                CheckerHelper4.CPM = CheckerHelper4.CPM_aux;
                CheckerHelper4.CPM_aux = 0;
                Console.Title = string.Format("Zyrotify | Module: Playlist Unfollower | Checked {0} - Remaining {1} | Hits {2} - Fails {4} - CPM ", new object[]
                {
                        CheckerHelper4.check,
                        CheckerHelper4.total,
                        CheckerHelper4.hits,
                        CheckerHelper4.free,
                        CheckerHelper4.bad,
                        CheckerHelper4.err,
                }) + CheckerHelper4.CPM * 60;
                Console.Clear();
                ...
                Console.Write("] CPM : ");
                Console.ForegroundColor = ConsoleColor.Cyan;
                Console.Write(CheckerHelper4.CPM * 60);
                Console.WriteLine("");
                ...
                Console.Write("] Error : ");
                Console.ForegroundColor = ConsoleColor.DarkYellow;
                Console.Write(CheckerHelper4.err);
                Console.WriteLine("");
                ...
                Console.ForegroundColor = ConsoleColor.DarkMagenta;
                Console.Write(CheckerHelper4.check); Console.Write(" / "); Console.Write(CheckerHelper4.total);
                Console.WriteLine("");
                ...
                Console.Write(">>>");
                Console.ForegroundColor = ConsoleColor.White;
                Console.Write("] UNFOLLOWED : ");
                Console.ForegroundColor = ConsoleColor.Green;
                Console.WriteLine(CheckerHelper4.hits);
                ...
                Console.Write("] BAN : ");
                Console.ForegroundColor = ConsoleColor.DarkRed;
                Console.WriteLine(CheckerHelper4.free);
                ...
                Console.Write("] BADS : ");
                Console.ForegroundColor = ConsoleColor.DarkRed;
                Console.WriteLine(CheckerHelper4.bad);
        ...
        public static void Check()
        {
            for (; ; )
            {
                if (CheckerHelper4.proxyindex > CheckerHelper4.proxies.Count<string>() - 2)
                {
                    CheckerHelper4.proxyindex = 0;
                }
                try
                {
                    Interlocked.Increment(ref CheckerHelper4.proxyindex);
                    using (HttpRequest req = new HttpRequest())
                    {
                        if (CheckerHelper4.accindex >= CheckerHelper4.accounts.Count<string>())
                        {
                            CheckerHelper4.stop++;
                            break;
                        }
                        Interlocked.Increment(ref CheckerHelper4.accindex);
                        string[] array = CheckerHelper4.accounts[CheckerHelper4.accindex].Split(new char[]
                        {
                            ':',
                            ';',
                            '|'
                        ...
                        try
                        {
                            if (CheckerHelper4.proxytype == "HTTP")
                            {
                                req.Proxy = HttpProxyClient.Parse(CheckerHelper4.proxies[CheckerHelper4.proxyindex]);
                                req.Proxy.ConnectTimeout = 5000;
                            }
                            if (CheckerHelper4.proxytype == "SOCKS4")
                            {
                                req.Proxy = Socks4ProxyClient.Parse(CheckerHelper4.proxies[CheckerHelper4.proxyindex]);
                                req.Proxy.ConnectTimeout = 5000;
                            }
                            if (CheckerHelper4.proxytype == "SOCKS5")
                            {
                                req.Proxy = Socks5ProxyClient.Parse(CheckerHelper4.proxies[CheckerHelper4.proxyindex]);
                                req.Proxy.ConnectTimeout = 5000;
                            }
                            if (CheckerHelper4.proxytype == "NO")
                            {
                                req.Proxy = null;
                            }
                            if (CheckerHelper4.check >= CheckerHelper4.total)
                            {
                                break;
                            }
                            ...
                            req.Delete("https://api.spotify.com/v1/playlists/" + Program.alqwe + "/followers").ToString();

                            if (giris.Contains("{\"result\":\"ok\",\""))
                            {

                                CheckerHelper4.CPM_aux++;
                                CheckerHelper4.check++;
                                CheckerHelper4.hits++;
                            }
                            ...
                            else if (giris.Contains("{\"error\":\"server_error"))
                            {
                                ...
                                CheckerHelper4.check++;
                                CheckerHelper4.free++;
                                CheckerHelper4.Free(text);

                            }
                            else
                            {
                                CheckerHelper4.accounts.Add(text);
                            }
                        }
                        catch (Exception)
                        {
                ...
                {
                    Interlocked.Increment(ref CheckerHelper4.err);
                }
            }
        }
        ...
        public static void Good(string account)
        {
            try
            {
                using (StreamWriter sw = File.AppendText("results/hotmail/Good.txt"))
                {
        ...
        public static void Bad(string account)
        {
            try
            {
                using (StreamWriter sw = File.AppendText("results/hotmail/Bad.txt"))
                {
                    sw.WriteLine(account);
                }
            }
        ...
        public static void Free(string account)
        {
            try
            {
                using (StreamWriter sw = File.AppendText("results/hotmail/2FA.txt"))
                {
                    sw.WriteLine(account);
                }
            }
